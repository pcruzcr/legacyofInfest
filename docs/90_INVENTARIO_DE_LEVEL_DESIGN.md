...
| 4-1b Mina inundada (variante) | `4_1_b.mp3` (material de autor, AUD-575) | mina, congelado, `ambient_light=0.45` (AUD-574) | tinte de agua `#1a5c6e`; `vida_abisal` | estalactitas; luces warm/blood/white; señuelo del pez | 1 + fauna (pez, cangrejos, medusas — sin daño) | `WaterZone` con superficie en la fila 11; oxígeno 30 s con aviso de HUD (GAP-071); bloques de mineral ×9; corrientes ×5 |
| 4-1c Niebla (variante) | `bgm_zone1_traverse` | cielo, congelado | — | `RhythmBlock` (rectángulo sólido/contorno según `presente`) | **ninguno** | `RhythmBlock` sincronizado con `bpm`/`compas`; 3 plantillas sorteadas por entrada |
