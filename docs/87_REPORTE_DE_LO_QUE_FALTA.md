...
| **3. Física ampliada** | Fricción por superficie desde el TMX (materiales: hielo, arena…), aceleración/fricción por perfil, modo `vuelo` integrado (8 direcciones sin gravedad) | Vuelo **HECHO (AUD-335)**: preset `vuelo()` + integración sin gravedad servida por el jugador (misma física que la cenital: dos ejes libres, velocidad desde la entrada, sin inercia; el modo es el dato del perfil); el resolutor reserva cuestas y repisas para `plataformas` (en vuelo son terreno pintado); 10 pruebas nuevas (9 del modo + 1 del resolutor). Fricción por TMX y aceleración por perfil: **Pendiente** — necesitan el campo en el perfil y el consumidor a la vez |
