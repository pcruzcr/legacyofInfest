...
| **Normal mapping** | **HECHO (AUD-340)** — corregido el 2026-08-11 (AUD-414): esta fila decía «Pendiente — vive en la fase 5 (GPU)» y llevaba envejecida desde entonces, contradiciendo a §28.1 del **mismo documento**, que ya lo daba por hecho. `engine/render/normales.py` genera el atlas de normales del alfa del sprite y lo consume `gpu_sprite_batch.py`, que modela la luz con una ambiental, una direccional y hasta varias puntuales. Sin mapa de normales la normal sale plana, así que el camino sin arte propio sigue funcionando |
