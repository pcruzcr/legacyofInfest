...
* **ECS por debajo de la herencia.** `World`, componentes y sistemas con fases
  explícitas y borrado diferido. Las **26 clases de escenario de los
  estudiantes siguen funcionando sin tocar una línea**, que era la restricción.
...
**62 tipos de objeto**, 17 propiedades de mapa, 8 capas. Incluye las once
...
post-procesado se hace **en CPU sobre superficies**, con `gl_pipeline.py` (479
líneas con sus sombreadores) ya escrito y sin usarse para esto.

### B2. `stage_scene.py` — 1.549 líneas
...
`mypy` entró en CI esta semana con **2 paquetes de unos 15**. El trinquete
existe justamente para ir subiendo; el trabajo es real pero mecánico.
...
Los catálogos de interfaz sí están completos (2.148 literales, `es` y `en`).
...
### B8. Quince tipos de objeto que ningún mapa usa

Diez de ellos son enemigos del bestiario oficial. Existir en el registro y no
aparecer en ningún mapa es el mismo huérfano de siempre, con otra ropa.
...
Lo que hay son 479 líneas de tubería GL para post-procesado, **no un
...
`61_AUDITORIA_AAA_2026-08.md`; la media alcanzable es **94**.
...
|---|---|---|---|
| Motor y arquitectura | ECS, 3 relojes, escalas componibles | `stage_scene` de 1.549 líneas | — |
| Jugador | 26 estados | — | — |
| Enemigos | 30 tipos, 13 estados, IA por lote | 15 tipos sin usar en ningún mapa | — |
| Jefes | fases, telegrafiado, puntos débiles | variedad entre jefes | — |
| Escenarios | 62 tipos TMX, 11 mecánicas | stage 0 usa 4 de 11 | — |
...
| Calidad | 1.608 pruebas, CI con 5 puertas | cobertura, mypy, docs atadas | mutación, resistencia |
