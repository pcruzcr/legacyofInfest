...
| ~~P2~~ | ~~Baja~~ | ~~`speedrun_mode.py:249`~~ *(Resuelto: AUD-363)* | `_frames` guarda `x`, `y` (números) y `state` (cadena) y estaba anotado `list[dict[str, float]]`: mentía en uno de sus tres campos. Con el arreglo, mypy pasa limpio sobre el fichero — candidato para ampliar el trinquete |
| ~~P3~~ | ~~Baja~~ | ~~`docs/00_MASTER_INDEX.md`~~ *(Resuelto: AUD-365)* | Decía 69 y la tabla tenía 75 filas: **dos formas de contar** sin declarar cuál. Medido: 71 `.md`, 70 con fila (el índice no se indexa a sí mismo), 5 filas de la raíz. Corregido y hecho recontable — `tests/test_el_indice_maestro_cuenta_bien.py` |
