...
| ~~Árbol de habilidades~~ | progresión | **HECHO (AUD-293).** Tres ramas de estadística con tope —diez corazones, +40 % de daño, doble de ultimate—, pantalla propia y rangos persistidos en el slot. Las mecánicas siguen viniendo de los jefes (AUD-294): lo que puedes *hacer* lo abre derrotar a alguien; lo que *aguantas o pegas*, jugar |
| ~~`LoadingScene`~~ | escenas | **HECHO (AUD-288).** Era huérfana de verdad —esta fila la daba por falso positivo y no lo era—. Ya tiene llamante: el menú de demos abre con ella el laboratorio de la Unidad IX, que **congelaba el juego 2.461 ms** importando scikit-learn y cargando el modelo en el hilo del dibujado. Precalentado, ese `on_enter` tarda 2 ms. No se enganchó a la transición entre escenarios porque medido no procede: entrar en un escenario cuesta 41–134 ms y una pantalla de carga tan corta parpadea |
