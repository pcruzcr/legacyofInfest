...
| `MusicZone` | Rectángulo | `track` (nombre de pista sin extensión; cadena vacía = silencio deliberado) · `fundido_ms` (entrada con fundido, 800). Mientras el jugador esté dentro, la sección manda sobre la intensidad de combate; al salir vuelve la base del mapa (AUD-600) |
| `CameraZoomZone` | Rectángulo | `factor` (>1 acerca, <1 aleja; saturado 0.4-2.5, 0.75) · `segundos` (duración del tween, 1.5). Dentro del rectángulo la cámara tiende al factor; fuera vuelve a 1.0. La UI nunca escala (AUD-601) |
