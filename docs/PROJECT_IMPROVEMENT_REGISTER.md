...
| **I-007c** | `CONTENT/P2` | `B4.1 Bonfire` Fogata reutilizable `heal 5.0 + checkpoint fogata` | Infra existe, certificar TMX Fogata/Bonfire + heal cap + reusable + B3 exclusion | Alto (checkpoint jugable) | Bajo 0 core + 14 tests | Nulo | **P1** | **FIXED** 2026-09-02 B4.1 COMPLETE — `Fogata/Bonfire TMX + _usar_fogata heal5+checkpoint+SFX` 14/14 PASS, `docs/features/bonfire.md` |
| **I-007d** | `CONTENT/P2` | `B4.2 HeartPiece` `heart_piece 0.25` + `4→vessel` craft | Infra existe (Inventory 0.25 + crafting 4→1), certificar TMX Pickup heart_piece + max/current health + B3 1 ITEM + anti-duplicate + slot/NG+ | Alto (vida progresiva) | Bajo 0 core + 21 tests | Nulo | **P1** | **FIXED** 2026-09-02 B4.2 COMPLETE — `Pickup heart_piece 21/21 PASS, max +0.25, craft 4→1 neto 0, docs/features/heart_piece.md` |
| **I-007e** | `CONTENT/P2` | `B4.3 RechargeStation` recarga estamina/especial reutilizable | Infra nueva mínima (EstacionDeRecarga + _usar_estacion), TMX alias, no checkpoint, B3 exclusion, reusable | Alto (recursos combate) | Bajo 3 ficheros + 11 tests | Nulo | **P1** | **FIXED** 2026-09-02 B4.3 COMPLETE — `RechargeStation TMX + _usar_estacion estamina/special max` 11/11 PASS, `docs/features/recharge_station.md` |
| **I-007** | `CONTENT/P2` | `B1-B4` riqueza barata `FEATURE→RULE→DATA→UI→SAVE→WORLD→TEST→PLAYTEST` | Aumentar rejugabilidad sin arquitectura | Alto | Medio 4 features secuenciales | Medio | **P1 después Playtest** | HOLD (B2+B3+B4.1-3 DONE, falta B1) |
