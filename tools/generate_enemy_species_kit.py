    ...
    if "Flying" in species_id or base_palette is PAL_ENEMY_FLYING:
        ...
        for f in range(4):
            ...
            def draw(wing_phase=wing_phase, pal=pal):
                pts=[]; cx,cy=12,12  # noqa: E702
                for dy in range(-3,4):
                    for dx in range(-3,4):
                        if abs(dx)+abs(dy)<4: pts.append((cx+dx, cy+dy, 2))  # noqa: E701
                wing_y=int(math.cos(wing_phase)*4)
                for dx in range(-8,-2): pts.append((cx+dx, cy+wing_y, 4))  # noqa: E701
                for dx in range(3,9): pts.append((cx+dx, cy+wing_y, 4))  # noqa: E701
        ...
        for f in range(6):
            ...
            def draw(phase=phase, pal=pal):
                pts=[]; cx,cy=10,8  # noqa: E702
                for dy in range(-5,7):
                    for dx in range(-9,10):
                        if abs(dx)+abs(dy)<11: pts.append((cx+dx, cy+dy, 2 if dy<0 else (3 if dy<3 else 4)))  # noqa: E701
                pts.append((cx-3, cy-2, 5)); pts.append((cx+3, cy-2, 5))  # noqa: E702
                swing=int(math.sin(phase)*2)
                pts.append((cx-4, cy+7, 6)); pts.append((cx+4+swing, cy+7, 6))  # noqa: E702
                # shooter adds weapon hint
                if "Shooter" in species_id or base_palette is PAL_ENEMY_SHOOTER:
                    pts.append((cx+8, cy, 7)); pts.append((cx+9, cy, 7))  # noqa: E702
                return pts
...
def gen_hurt(pal, size):
    ...
    for f in range(3):
        ...
        for x in range(w):  # noqa: B007
            for y in range(h):  # noqa: B007
                pass
        # simpler: solid tint rect with outline
        from PIL import ImageDraw
        d=ImageDraw.Draw(img)
        d.rectangle([2,2,w-3,h-3], fill=col+(255,), outline=(0,0,0,255))  # noqa: RUF005
        frames.append(img)
    return frames
...
def gen_die(pal, size):
    ...
    for f in range(5):
        ...
        shrink=f*2
        d.rectangle([2+shrink//2,2+shrink//2,w-3-shrink//2,h-3-shrink//2], fill=pal["colors"][3]+(alpha,), outline=(0,0,0,alpha))  # noqa: E501
        frames.append(img)
    return frames
...
for zone in [1,2,3]:  # noqa: B007
    for base_name, base in [("walker", PAL_ENEMY_WALKER), ("flying", PAL_ENEMY_FLYING), ("shooter", PAL_ENEMY_SHOOTER)]:  # noqa: B007
