...
class SpriteAtlas:
    ...
    def export_all(self, output_dir: Path, base_name: str = "atlas"):
        """Export to all supported formats"""
        atlas_path = output_dir / f"{base_name}.png"
        manifest_path = output_dir / f"{base_name}.json"
        godot_path = output_dir / f"{base_name}.tres"
        texturepacker_path = output_dir / f"{base_name}.tpsheet.json"
        
        atlas, manifest = self.build()
        atlas.save(atlas_path)
        with open(manifest_path, 'w') as f:
            json.dump(manifest, f, indent=2)
        export_godot_spriteframes(manifest, atlas_path, godot_path)
        export_texturepacker(manifest, atlas_path, texturepacker_path)
