...
class BossReyScene(StageScene):
    STAGE_ID: str = "stage2_4"
    STAGE_NAME: str = "REY TERCIOPELO"
    ZONE: int = 2
    TMX_PATH = settings.ASSETS_DIR / "maps/stage2_4/stage2_4.tmx"
    ...
    def on_enter(self) -> None:
        ...
        # Sin parallax en las capas de fondo: esto es un interior.
        #
        # El motor las dibuja por defecto a 0.15 / 0.40 / 0.70 de la velocidad
        # de la cámara, que es lo correcto para un paisaje abierto —montañas
        # lejanas que apenas se mueven—. Pero aquí el «fondo» es la pared de
        # una sala cerrada, con sus racks y sus consolas atornillados a ella:
        # al desplazarse la cámara, esos elementos se despegaban del suelo y
        # parecían flotar y moverse solos por la sala.
        #
        # `set_parallax_factor` es API pública de `Camera`, así que no hace
        # falta parchear el motor: se le dice que estas tres capas están a la
        # misma distancia que el terreno.
        for capa in ("BG_Far", "BG_Mid", "BG_Near"):
            self._camera.set_parallax_factor(capa, 1.0)
