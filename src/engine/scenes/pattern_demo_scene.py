...
class PatternDemoScene(BaseScene):
    ...
    def _render_inference(self, probas: dict[str, float], label: str, method: str,
        ...
        info = self._font_small.render(
            f"  Model: {self._model_name}  |  Method: {method}  |  Vector: {len(features)}", True, COLOR_TEXT,
        )
        surf.blit(info, (10, y))
    ...
    def _render_feature_compare(self, features: np.ndarray, nearest_feat: np.ndarray,
        ...
        y += 30
        info = self._font_small.render(
            f"  Distance: {distance:.3f}  |  Nearest: {nearest_label}", True, COLOR_TEXT,
        )
        surf.blit(info, (10, y))
    ...
    def _render_confusion(self) -> pygame.Surface:
        ...
        if not ev or "confusion_matrix" not in ev:
            msg = self._font_small.render(
                "Confusion matrix not available — run evaluate() during training", True, COLOR_TEXT,
            )
            surf.blit(msg, (10, 30))
            return surf
        ...
        acc_text = self._font_small.render(
            f"  Accuracy: {accuracy:.1%}  |  Classes: {n}", True, COLOR_HIGHLIGHT,
        )
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        src_label = self._font_small.render(
            f"  {self._sources.current_name}{' [FROZEN]' if self._sources.is_frozen else ''}  ", True, COLOR_ACCENT,
        )
