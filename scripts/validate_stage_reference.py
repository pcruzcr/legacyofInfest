...
def check_tmx(path, expected_w, expected_h, expected_ground_y):
    tree=ET.parse(path)
    el=tree.getroot()
    w=int(el.get("width")); h=int(el.get("height")); tw=int(el.get("tilewidth"))
    ...
    for name in ["BG_Far","BG_Mid","BG_Near","Terrain","Collision","Objects","FG_Overlay"]:
        assert el.find(f".//layer[@name='{name}']") is not None or el.find(f".//objectgroup[@name='{name}']") is not None, f"{path} missing {name}"
    # objects
    ...
    assert abs((y+64) - expected_ground_y) <= 2, f"{path} spawn {y}+64 != ground {expected_ground_y} delta {y+64 - expected_ground_y}"
    # checkpoint
